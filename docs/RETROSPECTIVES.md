...
### Retrospective — 2026-08-21 (full): docs drift sweep + compliance-audit skill

Changes analyzed: comprehensive docs-drift audit across all 21 `.md` files plus `.skills/` READMEs. 12 issues found (6 high, 5 medium, 1 low) covering Frozen-Flask stale references (Frozen-Flask was purged in the compliance batch but 4 docs still mentioned it), Alembic/migration cross-references (all correctly marked as historical — no action needed), duplicate Covers header in DEVELOPMENT_PROCESS.md, a CI runtime contradiction (DEVELOPMENT_PROCESS.md said ~10min, QUALITY_MANAGEMENT.md said ~3min; reconciled to ~10min/600s), a stale pyright-ignores count (92→93), B15/B16 out-of-sequence numbering in RELEASE_CHECKLIST.md, and a Frozen-Flask row in TESTING.md's exclusion table. Also: created `.skills/compliance-audit/README.md` — a reusable post-deploy compliance verification tool that checks security headers, consent banner, privacy page content, data-subject rights, form notices, and dormancy of third-party services against the live production site.

| Gap | Root cause | Fix |
| --- | ---------- | ---- |
| 4 docs referenced Frozen-Flask as if still present (ARCHITECTURE.md Static Site Generation section, DEVELOPMENT_PROCESS.md CLI quick-ref + release step, REQUIREMENTS.md section 5 deployment, TESTING.md exclusions table) | Frozen-Flask was removed in the compliance batch (PR #18/#237) but none of the doc-sweep commits scanned for stale references to the purged dependency. The compliance batch's changelist focused on what was added/changed, not what was removed | Removed all 4 stale references; ARCHITECTURE.md's scope-violating technology-choice section deleted entirely |
| DEVELOPMENT_PROCESS.md had duplicate Covers blocks (line 5 scope header + line 71 second Covers with different scope claims) | The second Covers block was left over from a previous restructuring that added a new scope header without removing the old inline one | Removed the duplicate on line 71; the canonical scope header on line 5 is the single source of truth |
| CI runtime estimate contradicted between DEVELOPMENT_PROCESS.md (~10min) and QUALITY_MANAGEMENT.md (~3min) | Both were written independently without cross-referencing each other; the 3min estimate predates the full pytest suite growth to 1402 tests | Replaced ~3min with ~10min in QUALITY_MANAGEMENT.md; DEVELOPMENT_PROCESS.md already had ~600s which was updated to explicitly say ~600s / ~10min |
| pyright-ignores count was 92 but actual count is 93 | Another session added an ignore (EnsureSchemaError or import fix) but QUALITY_MANAGEMENT.md was not updated | Updated to 93 |
| B15 (strict signing gate) appears after B16 in RELEASE_CHECKLIST.md | B15 was inserted after B16 during a previous edit without reordering | Swapped B15 before B16 so numeric sequence is correct |
| No structured compliance-audit workflow existed — each deploy required ad-hoc checks with no checklist | The compliance verification was implicit (docs/PRIVACY_COMPLIANCE.md section 4.7 criteria) but not repeatable by an agent | Created `.skills/compliance-audit/README.md` with a 9-step workflow covering security headers, consent banner, privacy page, data-subject rights, form notices, dormancy checks, and a pass/fail report template |

**Pattern recurrence**: NO.

**What went well**: the systematic grep across all docs caught 9 stale references that individual doc-reading would have missed; the dedicated doc-audit agent found structural issues (duplicate Covers, numbering gaps, scope violations) that go beyond stale text; the compliance-audit skill codifies the ad-hoc checks we ran after deploy into a repeatable, loadable workflow.

**What went wrong**: every high-severity issue was a Frozen-Flask stale reference — a pattern that shows removal PRs need a dedicated doc-sweep checklist step. The 2026-08-21 docs checkpoint commit (81da2b7) had already touched ARCHITECTURE.md but missed its Frozen-Flask section, proving that targeted edits miss what's near the edit boundary. The CI runtime contradiction and pyright-ignores drift both show that QUALITY_MANAGEMENT.md metrics decay silently between sessions.

**Fix**: all 12 issues fixed in this commit. The `compliance-audit` skill provides a repeatable post-deploy gate. Added to prevention baseline: after removing a dependency or feature, grep ALL `.md` files for the feature name before closing the session.

**State at handoff**: local staging synced. All 12 doc-drift fixes and the compliance-audit skill committed. Next: pre-push gate green -> push -> fork PR (base staging) -> ci-staging green -> squash-merge -> push origin/staging -> upstream PR #237 includes the fixes.
